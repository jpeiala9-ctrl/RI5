...
const Training = {
  startCalc() {
    ...
    if (!age || isNaN(time)) {
      alert("> COMPLETA EDAD Y TIEMPO CORRECTAMENTE");
      return;
    }
    
    this.calculate(age, time);
  },
  ...
  calculate(age, time) {
    const username = document.getElementById('name').value || 'Runner';
...
window.changeDailyTip = () => {
  ...
  if (e) e.innerHTML = '<span>> CONSEJO DEL DÍA_</span><small>// pulsa para nuevo consejo</small>';
};
window.changeConsejo = () => {
  const e = document.getElementById("curiosity");
  if (e) e.innerHTML = '<span>> CONSEJO_</span><small>// pulsa para nuevo consejo</small>';
};
